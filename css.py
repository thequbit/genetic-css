...
class Covering(set):
    ...
    def crossover(self, other):
        ...
        ob2 = set(other.cover(set(self.graph.edges) -
                              set().union(*(set(b.edges)
                                            for b in sb1))))

        ob1 = other - ob2
        sb2 = set(self.cover(set(other.graph.edges) - set().union(*(set(b.edges)
                                                                    for b in ob1))))
    ...
    def mutate_merge(self):
        ...
        try:
            ...
            b2 = random.sample(set(b2 for b2 in self
                                   if b1.can_merge(b2) and b1 != b2), 1)[0]
            self.remove(b1)
            self.remove(b2)
    ...
    def __init__(self, bicliques, gzip=0):
        ...
        for bl, br in bicliques:
            for l in bl:
                for r in br:
                    self.connect(l, r)
            self.base_covering.add(BiClique(self.base_covering, (self.nodes[node]
                                                                 for node in (bl | br))))
    ...
    def compress(self):
        ...
        for i in range(self.max_steps):
            ...
            while len(nextgen) < self.population_size:
                c1 = select(population, w, s)
                c2 = select(population, w, s)
                c1, c2 = crossover(self.crossover_ratio, c1, c2)
                mutate(self.mutation_ratio, c1)
                mutate(self.mutation_ratio, c2)
                nextgen.append(c1)
                nextgen.append(c2)
